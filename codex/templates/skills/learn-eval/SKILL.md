...
2. Run the `$review` skill's file classification logic to determine which reviewers would trigger
3. For each triggered reviewer, read its prompt from `~/.codex/skills/review/reviewers/{name}.md`
...
Write the scorecard to `~/.codex/learnings/review-evals/{repo}-{date}.md` for future reference.

If the learnings database (`~/.codex/learnings/learnings.jsonl`) exists, append any blind spots as new learnings entries so the `learnings-check` reviewer can catch them in future reviews.
