---
name: review
description: Comprehensive code review for NetDocuments AI services using specialized reviewer prompts and adversarial dual-pass reconciliation.
---
...
For each selected reviewer, read its prompt from `reviewers/{name}.md` relative to this skill directory. Launch a background Codex subagent with that prompt. Run the reviewer passes in parallel.
...
2. For each triggered reviewer, read its prompt from the installed `reviewers/{name}.md` files for this skill
